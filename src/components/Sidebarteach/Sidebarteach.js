...
export default function Sidebarteach({open,onContentClick}) {
  let {logoutUser, proxy} = useContext(AuthContext);
  return (
    <div className={open?"sidebar collapse":"sidebar"}>
      <button>Dashboard</button>
      <ul className='content'>
      <li onClick={() => onContentClick('home2')}>< FontAwesomeIcon className="menu" icon={faHome}/>Home</li>
